...
void main() {
  Map<String, dynamic> baseJson() => {
        'id': 1, 'title': 'Dev', 'company': 'Acme', 'location': 'NYC',
  ...
  group('JobListing.fromJson', () {
    ...
    test('parses skills from list', () {
      final j = JobListing.fromJson({...baseJson(), 'skills_required': ['Dart', 'Flutter']});
  ...
  group('JobListing boolean helpers', () {
    test('isRemote / isHybrid / isFullTime', () {
      ...
      final f = JobListing.fromJson({...baseJson(), 'employment_type': 'full_time'});
      expect(f.isFullTime, isTrue);
    });
    test('hasLocation', () {
      final j = JobListing.fromJson({...baseJson(), 'latitude': 4.05, 'longitude': 9.7});
      expect(j.hasLocation, isTrue);
    });
    test('hasContact', () {
      final j = JobListing.fromJson({...baseJson(), 'contact_email': 'a@b.com'});
  ...
  group('JobListing.salaryText', () {
    test('not specified', () {
      expect(JobListing.fromJson(baseJson()).salaryText, 'Salary not specified');
    });
    test('range with K formatting', () {
      final j = JobListing.fromJson({...baseJson(), 'salary_min': 50000, 'salary_max': 80000});
  ...
  group('JobListing display getters', () {
    test('experienceLevelDisplay maps', () {
      final j = JobListing.fromJson({...baseJson(), 'experience_level': 'entry'});
      expect(j.experienceLevelDisplay, 'Entry Level');
    });
    test('employmentTypeDisplay title-cases', () {
      final j = JobListing.fromJson({...baseJson(), 'employment_type': 'full_time'});
  ...
  group('JobListing equality', () {
    test('equal by id', () {
      expect(JobListing.fromJson(baseJson()), equals(JobListing.fromJson(baseJson())));
    });
    test('toString contains id/title/company', () {
      expect(JobListing.fromJson(baseJson()).toString(), 'JobListing(1: Dev @ Acme)');
    });
  });

  group('JobApplication', () {
    Map<String, dynamic> appJson() => {
          'id': 1, 'job_id': 2, 'status': 'pending',
          'applied_at': '2025-01-01 10:00:00', 'title': 'Dev', 'company': 'Acme',
        };
    ...
    test('statusDisplay maps', () {
      expect(JobApplication.fromJson(appJson()).statusDisplay, 'Pending Review');
